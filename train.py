...
def train_epoch(epoch, model, config, train_loader, criterion, optimizer):
    ...
    t = tqdm(train_loader, desc=message)
    ...
    for _ , (inputs, target, temp, mask_rec, flows) in enumerate(t):
        ...
        if(config.loss == 'Rec&Tem'):
            ...
            loss_tem = criterion(output_ill, temp_warped_ill)
    ...
    if(config.loss == 'Rec&Tem'):
        train_loss.append(loss_tem_sum/loss_cnt)
        train_loss.append(loss_sum/loss_cnt)
